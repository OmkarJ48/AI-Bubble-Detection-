...
def match_bubbles(detected, previous, max_dist=70):
    """Match new detections to existing tracked bubbles."""
    global bubble_id_counter, bubble_count_total
...
def frame_capture_thread():
    ...
    global tracked_bubbles, bubble_count_total, counted_ids
    frame_count = 0
    ...
    while streaming:
        ...
        try:
            ...
            for bid, (cx, cy, r, t) in tracked_bubbles.items():
                if cy <= PIPE_EXIT_Y and bid not in counted_ids:
                    bubble_count_total += 1
                    counted_ids.add(bid)
                    print(f"COUNTED {bid} total={bubble_count_total}")
            ...
            for bid in list(tracked_bubbles.keys()):
                ...
                if now - t > 3.0:
                    ...
                    counted_ids.discard(bid)

            # Draw tracked bubbles
            for bid, (cx, cy, r, t) in tracked_bubbles.items():
                cv2.circle(frame, (cx, cy), 10, (0, 255, 0), 2)
                cv2.putText(
                    frame, str(bid), (cx + 10, cy),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1
                )

            print(f"Tracked: {len(tracked_bubbles)} | Total Count: {bubble_count_total}")
